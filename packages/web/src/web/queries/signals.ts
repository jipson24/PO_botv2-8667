...
export function useSignals(input: { limit?: number; direction?: "call" | "put"; symbol?: string } = {}) {
  return useQuery(
...
export function useScanRuns(limit = 12) {
  return useQuery(orpc.signals.runs.queryOptions({ input: { limit }, refetchInterval: 15_000 }));
}
